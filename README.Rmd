...
subset2: verwendet base::subset + label_data_frame
  
filter2: verwendet dplyr::filter + info was gefiltert wurde
...
Formeln werden auf zwei Arten evaluiert

   1. `Wide(A ~ student)`  links steht `values_from` rechts `names_from`
   2. `Wide(month ~ student, A)` jetzt steht in der formula die ausgabe Strucktur  und `names_from` wird extra übergeben
 ...
 * String umbrechen:   split_string(), wrap_string(), wrap_sentence(), wrap_string_at()
